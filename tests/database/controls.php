...
gml_db_assert( get_option( 'gml_crawl_running' ) === true, 'scan start repairs a stale persistent alloptions value' );
$crawl_autoload = $wpdb->get_var( $wpdb->prepare( "SELECT autoload FROM {$wpdb->options} WHERE option_name = %s", 'gml_crawl_running' ) );
gml_db_assert( ! in_array( $crawl_autoload, [ 'yes', 'on', 'auto', 'auto-on' ], true ), 'crawl runtime state is not autoloaded' );
$alloptions = wp_cache_get( 'alloptions', 'options' );
gml_db_assert( ! is_array( $alloptions ) || ! array_key_exists( 'gml_crawl_running', $alloptions ), 'crawl runtime state is evicted from persistent alloptions' );
